...
const Terms = () => {
  ...
  const sections = {
    ...
    section7:
      ' Pour toute question ou assistance, vous pouvez nous contacter via : 📧 Email : support@movies.com  📞 Téléphone : +33 1 23 45 67 89 💬 Support en ligne : Chat disponible 24/7 Nous nous engageons à répondre sous 48 heures ouvrées.',
    section8:
      'Découvrez nos articles sur le cinéma, les critiques de films et les tendances du streaming sur notre blog officiel : [Lien vers le blog Movies] Nos thématiques incluent : 🎬 Les coulisses des grands films, 🍿 Les meilleures recommandations, 📢 Les dernières actualités du streaming. Rejoignez notre communauté et partagez votre passion du cinéma !'
  };

  const sectionDisplayNames = {
    section1: 'Introduction',
    section2: 'Utilisation de nos services',
    section3: 'Protection des données',
    section4: 'Droits et responsabilités',
    section5: 'Limitations et mises à jour',
    section6: 'Confidentialité',
    section7: 'Contact',
    section8: 'Blog'
  };
  ...
  return (
    <main className={mainTerms}>
      <section style={{ height: '300px' }} className={termsOne}>
        <div className="container-terms-left z-2">
          ...
          <p className="text-center text-lg-start">
            Cette page décrit les règles d&apos;utilisation de notre plateforme pour vous garantir
            une expérience sécurisée et de qualité. <br /> En l&apos;acceptant, vous utilisez notre
            service en toute confiance.
          </p>
        </div>
      ...
      <section className={termsTwo}>
        <section className="architecture-section d-flex flex-column flex-lg-row">
          <div className="architecture-sidebar bg-dark p-3 p-lg-5">
            <h2 className="px-3 px-lg-5 mb-4 text-center text-light">Architecture</h2>
            ...
            <div className="dropdown d-lg-none mb-4">
              <Button
                ...
                aria-expanded="false">
                {sectionDisplayNames[visible] || 'Menu'}
              </Button>
              <ul className="dropdown-menu dropdown-menu-dark w-100">
                {Object.keys(sections).map((sectionKey) => (
                  <li key={sectionKey}>
                    <button
                      className={`dropdown-item ${visible === sectionKey ? 'active' : ''}`}
                      onClick={() => setVisible(sectionKey)}>
            ...
            <ul
              className="list-group py-3 py-lg-5 d-none d-lg-block"
              style={{ borderRight: '2px solid var(--color-red)' }}>
              {Object.keys(sections).map((sectionKey) => (
                <li
                  key={sectionKey}
                  className={`list-group-item text-light bg-transparent border-0 py-2 
                          ${visible === sectionKey ? 'active-sidebar-item' : ''}`}>
                  <Button
                    children={sectionDisplayNames[sectionKey]}
                    onClick={() => setVisible(sectionKey)}
                    className="t-btn w-100 text-start"
                  />
                </li>
              ))}
            </ul>
          ...
          <div className="architecture-content p-3 p-lg-5 flex-grow-1">
            <p
              className="w-100 py-3 py-lg-5 fs-6"
              style={{
                textAlign: 'justify',
                lineHeight: '1.8em'
              }}>
